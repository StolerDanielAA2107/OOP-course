...
CSize Students_Stoler::drow(CDC* pDC)
{
	...
	// отступы горизонтальные //  отсупы между словами // высота линий
	...
	if (students.size() != 0)
	{
		...
		for_each(students.begin(), students.end(), [&](shared_ptr<Student_Stoler> student)
			{
				student->CDCDrow(pDC, aLeft, top, lineHeight);
			});
